...
After the HTML, output a new line with:
MERGED_USER_MESSAGE: <the merged string describing the latest intent clearly>
...
`;
		...
		for await (const chunk of completion) {
			...
			// capture merged message if present
		...
		const finalReply = fullText
			.replace(/MERGED_USER_MESSAGE:.*$/m, "") // remove merged line
			.replace(/###END###/g, "")
			.trim();

		// ✅ update session with merged message only (not included in HTML shown to user)
